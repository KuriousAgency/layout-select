 ...
 */
Kurious.LayoutSelectModal = Garnish.Modal.extend(
	{
		elementType: null,
		elementIndex: null,
		...
		init: function(items, settings) {
			...
			this.setSettings(settings, Kurious.LayoutSelectModal.defaults);

			//console.log(items);

			// Build the modal
			var $container = $('<div class="modal layoutselectmodal"></div>').appendTo(Garnish.$bod),
			...
			items.forEach(function(d) {
				body += '<div class="element" data-id="' + d.id + '"><img src="' + d.url + '" alt=""></div>';
			});

			this.$body.html(body);
			this.$elements = this.$body.find('.element');

			this.addListener(this.$cancelBtn, 'activate', 'cancel');
			this.addListener(this.$selectBtn, 'activate', 'selectElement');

			this.addListener(this.$elements, 'tap', function(ev, touchData) {
				this.$selectedElement = $(ev.currentTarget);
				this.onSelectionChange();
			});

			if (this.settings.selectedElementId) {
				this.$selectedElement = this.$elements.filter('[data-id="' + this.settings.selectedElementId + '"]');
				this.onSelectionChange();
			}
			...
			this.addListener(this.$elements, 'doubletap', function(ev, touchData) {
				...
				if (touchData.firstTap.target === touchData.secondTap.target) {
					//console.log(ev.currentTarget.dataset.attr);
					this.$selectedElement = $(ev.currentTarget);
					this.selectElement();
				}
			});
		...
		selectElement: function() {
			if (this.$selectedElement) {
				...
				if (this.settings.disableElementsOnSelect) {
					this.disableElement(this.$selectedElement);
				}
		...
		onSelect: function() {
			...
			this.settings.onSelect(this.$selectedElement);
		},
